...
formCreate.addEventListener('submit', (e) => {
    ...
    formCreate.reset()
    if(todoText.length) {
        todos.push({text: todoText, time: dateTimeFunc(), completed: false})
        setTodos()
        showTodos()
    } else{
...
function dateTimeFunc() {
    ...
    fullDay.textContent = `${date} ${month_title}, ${year}`
...
formEdit.addEventListener('submit', (e) => {
    ...
    formEdit.reset()
    if(todoText.length) {
        todos.splice(editItemId, 1, {text: todoText, time: dateTimeFunc(), completed: false})
        setTodos()
        showTodos()
        close()
    } else{
